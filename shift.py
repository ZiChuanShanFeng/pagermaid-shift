...
@listener(
    ...
    parameters="set [from channel] [to channel] (silent) (keywords:[keyword1,keyword2,...]) 自动转发包含关键词的频道新消息（可以使用频道用户名或者 id）\n"
...
async def shift_set(client: Client, message: Message):
    ...
    # 列出要转存的频道
        ...
        from_ids = list(
            filter(
                lambda x: (x.startswith("shift.") and (not x.endswith("options")) and (not x.endswith("keywords"))),
                list(sqlite.keys()),
            )
        )
        if not from_ids:
...
async def shift_channel_message(message: Message):
    ...
    with contextlib.suppress(Exception):
        if keywords and any(re.search(keyword, message.text or "", re.IGNORECASE) for keyword in keywords):
